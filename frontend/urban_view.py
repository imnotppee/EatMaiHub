...
def build_urban_view(page: ft.Page) -> ft.View:
    ...
    if banner_images:
        banner_section = ft.Container(
            ...
            content=ft.Image(src=banner_images[0], fit=ft.ImageFit.COVER),
        )
    else:
        banner_section = ft.Container(width=PHONE_W, height=210, bgcolor="#EEE")
    ...
    def menu_card(item):
        return ft.Container(
            width=(PHONE_W - 64) / 2,
            height=190,  # เพิ่มความสูงให้ภาพใหญ่ขึ้น
